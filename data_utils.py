...
def _session(retries: int = 1) -> requests.Session:
    ...
    session.headers.update(
        {
            "User-Agent": "Macro-ML-Panel/3.1 (+quant-research; public-data-client)",
...
def get_bcb(code: int, start: str | None = None) -> pd.Series:
    """Load a real SGS series using the bounded `ultimos/N` endpoint.

    The SGS service explicitly supports this endpoint and it avoids the
    historical date-window failures/large responses that can occur with long
    dataInicial/dataFinal requests. Ten thousand observations cover the
    panel's 2000-present research window for all configured series.
    """
    session = _session(retries=2)
    base = f"{BCB_BASE}.{int(code)}/dados/ultimos/{BCB_LAST_N}"
    ...
    try:
        ...
        if series.empty:
            raise ValueError(
                f"nenhuma observação atende ao período solicitado ({requested_start:%Y-%m-%d} a {requested_end:%Y-%m-%d})"
            )
        series.name = str(code)
        return series
